    ...
    try:
        ...
        for atom in mol_h.GetAtoms():
            ...
            if atom.HasProp('_GasteigerCharge'):
                ...
                charges.append(charge if np.isfinite(charge) else 0.0)
            else:
                charges.append(0.0)
        return charges
    except Exception:
...
def featurize_ligand(smiles: str, use_3d: bool = False):
    ...
    atom_feats = np.nan_to_num(np.array(atom_feats, dtype=np.float32), nan=0.0, posinf=0.0, neginf=0.0)
    bond_feats = np.nan_to_num(np.array(bond_feats, dtype=np.float32), nan=0.0, posinf=0.0, neginf=0.0)
    ...
    if use_3d:
        ...
        coords = np.nan_to_num(np.array(heavy_coords, dtype=np.float32), nan=0.0, posinf=0.0, neginf=0.0)
        # Append xyz coordinates as extra atom features in 3D mode.
        atom_feats = np.concatenate([atom_feats, coords], axis=1)

    mol_desc = np.nan_to_num(np.concatenate([
        compute_molecular_descriptors(smiles),
        np.array([compute_forcefield_energy(mol, use_3d)], dtype=np.float32),
    ], axis=0), nan=0.0, posinf=0.0, neginf=0.0)
    return (
        fp_bits,
        atom_feats,
        coords,
