...
class LigandPocketNet(nn.Module):
    def __init__(self, lig_in=16, poc_in=25, int_in=25, edge_dim=10, int_edge_dim=3, hidden_dim=128, num_layers=3, dropout=0.1, mol_desc_dim=17):
        super().__init__()
        fusion_in_dim = 4 * hidden_dim + mol_desc_dim
        ...
        # Learnable fusion gate: weights combination of [h_l, h_p, h_i, interaction, desc]
        self.fusion_gate = nn.Sequential(
            nn.Linear(fusion_in_dim, fusion_in_dim),
            nn.Sigmoid(),
        )
        
        # FC layers with LayerNorm for stability
        self.fc_ln = nn.LayerNorm(hidden_dim)
        self.fc1 = nn.Linear(fusion_in_dim, hidden_dim)
        self.fc_residual = nn.Linear(fusion_in_dim, hidden_dim)
    ...
    def forward(self, lig_data, poc_data, int_data=None):
        ...
        # Learnable fusion: gate weights the concatenated representation
        gate = self.fusion_gate(h)
        h_fused = gate * h
        
        # FC with LayerNorm and residual connection
        h_fc = F.relu(self.fc1(h_fused))
        h_fc = self.fc_ln(h_fc)
        h_fc = self.dropout(h_fc)
        
        # Residual skip via projection so the skip path matches hidden_dim.
        h_fc = h_fc + self.fc_residual(h_fused)
        
        output = self.fc2(h_fc).squeeze(-1)
        return output
