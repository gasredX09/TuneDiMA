...
RUN_NAME="${RUN_NAME:-baseline_full_ft_ddp}"
...
BATCH_SIZE="$((BATCH_SIZE_PER_GPU * GPUS_PER_NODE))"
...
torchrun --nproc_per_node="$GPUS_PER_NODE" --master_port="$MASTER_PORT" "$PROJECT_ROOT/scripts/run_dima_train.py" \
  ddp.enabled=true \
